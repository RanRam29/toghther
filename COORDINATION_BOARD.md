...
> 🔴 **ממצא ארכיטקט (2026-07-19, במהלך גל G1) — דליפת D55 בשכבת הנתונים: `professional_stats_view` חושף `reporting_consistency_90d` להורים.**
> תוך re-skin/gap-fill של S-PAR-02 (`match-detail`) התגלה שהמסך ההורי הציג **עקביות דיווח (%)** של המשלבת — ו-D55 קובע במפורש: "עקביות דיווח מוצגת למשלבת עצמה ולאדמין בלבד, **לא פומבית**". **תיקנתי את ה-UI** (הסרתי את העמודה מ-`match-detail.tsx`; נשארו רק חודשי פעילות + ליוויים שהושלמו, הפומביים לפי D55). **אבל הבעיה עמוקה יותר:** `useProfessionalStats` קורא ישירות מ-`professional_stats_view`, שהוא `security_invoker=true` + `GRANT SELECT TO authenticated` וכולל את `reporting_consistency_90d` לכל משלבת — כלומר **כל משתמש מאומת (כולל הורה) יכול לקרוא את ה-% דרך ה-API**, גם אחרי תיקון ה-UI. ה-security review מ-`20260715150000` לא הסיר אותו.
> **תיקון נדרש (backend, טעון pgTAP — לא נכתב עיוור):** לפצל ל-view ציבורי (`months_active`+`completed_matches` בלבד) לכולם, ול-view/RPC מוגן (owner+admin) שמחזיר `reporting_consistency_90d`; לעדכן את הקורא בצד המשלבת שיקרא מהמוגן. **שים לב:** מסך הכלים של המשלבת עצמה קורא את ה-% — אין להסיר את העמודה בלי לפצל, אחרת יישבר. לא כתבתי מיגרציה כי היא נוגעת ב-view שכמה מסכים תלויים בו ודורשת בדיקת DB שלא ניתן להריץ מכאן.
> ---
...
- **חדש (2026-07-19)**: תיקון דליפת D55 בוצע. נוצרה מיגרציה לפיצול ה-view ועדכנתי את `useProfessionalTools.ts` ואת מסכי ה-UI הרלוונטיים. D55 עכשיו נאכף ב-DB. **בדיקות DB (pgTAP) נכתבו, אך הרצתן נכשלה עקב חסימת רשת Docker מקומית**. נדרשת פריסה לענן ובדיקה שם.
- **הצעד הבא**: ממתין לבדיקת בעל המוצר ולפתיחת פורטים מקומיים לפריסה לענן.
- **חסימות**: חסימת רשת מקומית של Docker עדיין מונעת db push והרצת טסטים מקומיים.
