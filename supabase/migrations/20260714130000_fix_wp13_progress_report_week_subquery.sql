  ...
  match_weeks AS (
    ...
    FROM (
      SELECT
        lw.match_id,
        lw.week_start,
        count(DISTINCT lw.log_date) as logs,
        avg(lw.mood) as mood_avg,
        (
          ...
          FROM (
            ...
            WHERE l2.match_id = lw.match_id
              AND (date_trunc('week', l2.log_date::timestamp + interval '1 day') - interval '1 day')::date = lw.week_start
              AND l2.log_date BETWEEN p_from AND p_to
              AND e.key = ANY(COALESCE((SELECT metric_keys FROM matches WHERE id = lw.match_id), '{}'::text[]))
            GROUP BY e.key
          ) agg
        ) as metrics_avg
      FROM (
      ...
      GROUP BY lw.match_id, lw.week_start
    ) w
    GROUP BY w.match_id
  )
